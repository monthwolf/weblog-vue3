<template>
  ...
  <main class="container max-w-screen-xl mx-auto p-4 px-6">
    ...
    <div class="grid grid-cols-4 gap-7">
        ...
        <div class="w-full p-5 mb-3 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
          ...
          <article>
            ...
            <div class="text-gray-400 flex items-center mt-5 text-sm">
              <!-- 发布时间 -->
              <svg class="inline w-3 h-3 mr-2 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
              ...
              <a @click="
                goCategoryArticleListPage(article.categoryId, article.categoryName)
      ...
      <AsideCard :showInfo="false"></AsideCard>
    </div>
  </main>
...
import hljsDefineVue from "highlightjs-vue/dist/highlightjs-vue.esm";
...
function refreshArticleDetail(articleId) {
  getArticleDetail(articleId).then((res) => {
    // 该文章不存在(错误码为 20005)
    if (!res.success && res.errCode == "20005") {
      // 手动跳转 404 页面
...
::v-deep(.article-content h1, .article-content h2, .article-content h3, .article-content h4, .article-content h5, .article-content h6) {
  color: #292525;
  line-height: 150%;
