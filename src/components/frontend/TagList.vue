<template>
    <div v-if="tags && tags.length > 0"
        ...
        <!-- 标签标题 -->
        <h2 class="mb-2 font-bold text-gray-900 uppercase dark:text-white w-full flex items-center cursor-pointer"
            @click="goTagListPage()">
            <!-- 标签图标 -->
            <svg t="1698980289658" class="icon w-5 h-5 inline-block mr-2" viewBox="0 0 1024 1024" version="1.1"
        ...
        <!-- 标签列表 -->
        <span v-for="(tag, index) in tags" :key="index" @click="goTagArticleListPage(tag.id, tag.name, tag.color)"
            :style="{ color: tag.color }"
            class="inline-block mb-1 cursor-pointer bg-green-100 text-xs font-medium mr-2 px-2.5 py-0.5 rounded hover:bg-green-200 hover:text-green-900 dark:bg-green-900 dark:text-green-300">
            {{ tag.name }}
        </span>
    </div>
</template>
