    ...
    >
      <template #content>
        ...
        <el-form-item label="标签颜色">
          ...
          />
          <div class="color-preview" :style="{ background: tagForm.color }">预览效果</div>
        </el-form-item>
      </template>
    </FormDialog>
      ...
      <template #content>
        <el-form-item label="标签列表" prop="tags">
          <el-input
            ...
            rows="6"
            placeholder="请输入标签名称，多个标签用逗号或换行分隔"
          />
          <div class="form-tip">已识别 {{ recognizedTags.length }} 个标签</div>
        </el-form-item>
        <div class="recognized-tags" v-if="recognizedTags.length">
          <el-tag
            v-for="tag in recognizedTags"
            :key="tag"
            class="mx-1"
            closable
...
const submitBatchTags = async () => {
  ...
  const tags = recognizedTags.value.map((tag) => ({
    name: tag,
